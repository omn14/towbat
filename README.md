...
texture, country meshes and collisions, and cloud resources are loaded only
on first entering the campaign view with F6, then reused. Normal battle startup
and scene tests both benefit; the campaign update tasks run only in that view.
...
The shared fixture in `tests/test_faction_rules_scene.py` skips figure artwork:
...
seconds in a local before/after run. Peak RSS fell from 420.5 to 404.6 MiB.
...
```bash
source .venv/bin/activate && python run_tests_isolated.py --memory-mb 512 tests/test_free_pivot_scene.py
```
