...
from panda3d.core import (
    Point3, Vec3, Vec4,
    CardMaker, TransparencyAttrib, NodePath, LineSegs,
)
...
_TERRAIN_COLORS = {
    'forest': Vec4(0.08, 0.35, 0.08, 0.45),
...
class TerrainPiece:
    ...
    # ── Visual: translucent coloured quad on the ground ───────────────

    def _create_visual(self):
        cm = CardMaker(f"terrain_{self.terrain_type}")
        cm.setFrame(-self.width / 2, self.width / 2,
                    -self.height / 2, self.height / 2)
        self.visual = render.attachNewNode(cm.generate())
        self.visual.setPos(self.center.x, self.center.y, 0.05)
        self.visual.setHpr(0, -90, 0)  # flat on the XY ground plane
    ...
    def _create_collision(self):
        ...
        self.ghost_np.setCollideMask(BitMask32.bit(20))
